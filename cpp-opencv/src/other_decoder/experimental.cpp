...
cv::Mat decodeH264ToMat(const uint8_t *h264Data, size_t length, int width, int height);
            ...
            if (verbose)
            {
                ...
                if (envelope.dataType() == 1055)
                {
                    ...
                    if ("h264" == img.fourcc())
                    {
                        ...
                        // Decode image
                        cv::Mat bgrImage = decodeH264ToMat(reinterpret_cast<const uint8_t *>(data.c_str()), data.size(), WIDTH, HEIGHT);
...
cv::Mat decodeH264ToMat(const uint8_t *h264Data, size_t length, int width, int height)
{
    ...
    // Set up the packet containing the H264 data
    av_init_packet(&pkt);
    pkt.data = const_cast<uint8_t *>(h264Data); // the data from your ImageReading message
    pkt.size = static_cast<int>(length);

    // Allocate the frame to store decoded video
    ...
    // Convert to BGR format using Swscale
    ...
    uint8_t *srcData[1] = {frame->data[0]};

    sws_scale(swsCtx, srcData, frame->linesize, 0, height, &rgbData, &frame->linesize[0]);
